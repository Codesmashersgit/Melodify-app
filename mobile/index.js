...
// Combined search — returns matching artists (with a quick way to load their
// songs) alongside matching tracks, so typing an artist name surfaces the
// artist first and typing a song name surfaces songs directly.
app.get('/api/search/all', async (req, res) => {
    const { query } = req.query;
    if (!query || !query.trim()) return res.json({ artists: [], tracks: [] });

    try {
        const [trackData, artistData] = await Promise.all([
            jiosaavnRequest({ __call: 'search.getResults', q: query, n: '20' }).catch(() => ({ results: [] })),
            jiosaavnRequest({ __call: 'search.getArtistResults', q: query, n: '6' }).catch(() => ({ results: [] })),
        ]);

        const tracks = (trackData.results || []).map(formatSong);
